...
void setup()
{
  ...
  ab_toneMutesScore(true);       // mute the score when a tone is sounding
#endif

  ab_invert(!arduboy.audio.enabled()); // invert display if sound muted
}
...
void loop()
{
  ...
  if (ab_pressed(UP_BUTTON)) {
    y-=1;
    ab_tone(1175,300);
  } else if (ab_pressed(DOWN_BUTTON)) {
    y+=1;
    ab_tone(1397,300);
  } else if (ab_pressed(LEFT_BUTTON)) {
    x-=1;
    ab_tone(1047,300);
  } else if (ab_pressed(RIGHT_BUTTON)) {
    x+=1;
    ab_tone(1319,300);
  }
