...
const char* TITEL1 = "Ardu";                                 // Titelbildschirm: Kleingedruckte erste Titelzeile
const char* TITEL2 = "BOYNG";                                // Titelbildschirm: Großgedruckte zweite Titelzeile
const char* UNTERTITEL = "v0.81";                            // Titelbildschirm: Untertitel
...
const bool SPIELFELDRAND = true;                           // Ob Spielfeldrand gezeichnet wird (true) oder nicht (true). Hinweis: Zur Zeit _mit_ Spielfeldrand programmiert. Für Variante ohne Spielfeldrand müsste (eigentlich) noch die Spielfeldgrenzen, Ball-/Schläger-Mini/Maxima umdefiniert werden.
...
const char* MENUE_TEXTE[][5] =                               // Menütexte Zeile für Zeile mit jeweils den auswählbaren Werten und der Zeilenbezeichnung
  {
...
const bool MENUE_MODE_WERTE[][2] = { {false, true}, {false, false}, {true, true} };  // Auswählbare Werte der Menüzeile Mode: {false, true} bedeutet linker Schläger vom Menschen gesteuert, rechter Schläger vom Computer gesteuert ("AI" = Artificial Intelligence). Usw.
...
const bool MENUE_SOUND_WERTE[] = { true, false };          // Auswählbare Werte der Menüzeile Sound: true = on, false = off
...
const char* MENUE_HAUPTZEILE_EINLEITUNG = ">> ";             // Text zur Markierung der Hauptzeile des Menüs (vor der Menüzeile), 3 Zeichen
const char* MENUE_HAUPTZEILE_ABSCHLUSS = "<<";               // Text zur Markierung der Hauptzeile des Menüs (hinter der Menüzeile), 3 Zeichen
...
// "AI"-Variable
bool ai1;                                                  // false: linker Schläger wird von Mensch gespielt. true: linker Schläger wird von Computer gespielt. Wird gesetzt nach Menüauswahl aus Menüzeile "Mode".
bool ai2;                                                  // false: rechter Schläger wird von Mensch gespielt. true: rechter Schläger wird von Computer gespielt. Wird gesetzt nach Menüauswahl aus Menüzeile "Mode".
...
bool sound;                                                // Töne an (true) oder aus (false). Default = false. Wird auf Default gesetzt vor Menüauswahl und während Menüauswahl je nach Einstellung geändert.
...
// Steuervariablen
bool gameover = false;                                     // Steuervariable: Falls ein Spieler die Gewinnpunktzahl erreicht, wird gameover=true gesetzt und damit die Gameoversequenz und der Neustart initiiert.
bool intro = true;                                         // Steuervariable: Ob der Titel- und Menüschirm angezeigt werden soll.
bool anstoss = true;                                       // Steuervariable: Am Spielbeginn und nach jedem Punktgewinn erfolgt ein Anstoss
...
void setze_variablen_vor_spielstart() {
  ...
  v = vInitial;
  phiDeg = ab_random(-PHI_DEG_MAX, PHI_DEG_MAX);
  phi = phiDeg / 180 * PI;
  dxb = v / sqrt( 1 + tan(phi)*tan(phi) );
  // dx_sign = (ab_random(0,2)*2-1);
  dxbVorzeichen = 1;
  if (ab_random(0,2) == 0) dxbVorzeichen = -1;
...
void zeige_startschirm_und_fuehre_menue_aus() {
  ...
  bool warteAufStarttaste = true;                          // true: Schleife zur Menüauswahl läuft bis Starttaste gedrückt (dann warteAufStarttaste=false)
  while ( warteAufStarttaste ){
    ...
    if ( ab_pressed(A_BUTTON) || ab_pressed(B_BUTTON) ) { 
      if (sound) ab_tone (TON_MENUE_FREQ, TON_MENUE_DAUER);
//      ab_delay(COUNTDOWN_VERZOEGERUNG);
    ...
    if ( ab_pressed(DOWN_BUTTON) ) {
      ...
      ab_delay(MENUE_VERZOEGERUNG); // Should be smarter debounce!!
    }
    ...
    ab_delay(50);                                                // Kurze Pause, bevor Tastenabfrageschleife wieder ausgeführt wird
  }
  ab_clear();
}  
...
void zeige_spielende() {
  ...
  ab_delay(GAMEOVER_VERZOEGERUNG);
}
...
void zeichne_spielfeld() {
  ...
  if (SPIELFELDRAND) {
    for ( int i = 0; i < WIDTH; i+=2 ) {
      ab_drawPixel( i, 0, WHITE ); 
      ab_drawPixel( i, HEIGHT - 1, WHITE );
    }
  }
  // Vertikale gepunktete Linien links, Mitte, rechts
  for ( int j = 0; j < HEIGHT; j+=2 ) {
    ab_drawPixel( WIDTH/2 - 1, j, WHITE );
    if (SPIELFELDRAND) {
      ab_drawPixel( 0, j, WHITE );
      ab_drawPixel( WIDTH - 1, j, WHITE );
    }
  }
...
void bewege_ball(){
  ...
  if ( yb > YB_MAX ) {                                        // Pralle vom unteren Rand ab
    if (sound) ab_tone(TON_RAND_FREQ,TON_RAND_DAUER);
    dyb = -dyb;
    yb = YB_MAX;
  }
...
void male_ball(bool color) {
  for ( int i = 0; i < BALL_DURCHMESSER; i++ ) {              // Ball besteht aus BALL_DURCHMESSER Zeilen...
    for ( int j = 0; j < BALL_DURCHMESSER; j++ ) {            // ... und BALL_DURCHMESSER Spalten...
      ab_drawPixel( xb+i, yb+j, color );                 // ... von weißen Pixeln (oder schwarz beim Übermalen = Löschen).
    }
  }
}
...
void bewege_schlaeger() {
  ...
  if ( ab_pressed(UP_BUTTON) ) {                         // Bewege linken Schläger hoch
    ys1=ys1-DYS1;
    if ( ys1 < YS_MIN ) ys1 = YS_MIN;
  }
  if ( ab_pressed(DOWN_BUTTON) ) {                       // Bewege linken Schläger runter
    ys1=ys1+DYS1;
    if ( ys1 > YS_MAX ) ys1 = YS_MAX;
  }
  if ( ab_pressed(B_BUTTON) ) {                          // Bewege rechten Schläger hoch
    ys2=ys2-DYS2;
    if ( ys2 < YS_MIN ) ys2 = YS_MIN;
  }
  if ( ab_pressed(A_BUTTON) ) {                          // Bewege rechten Schläger runter
    ys2=ys2+DYS2;
    if ( ys2 > YS_MAX ) ys2 = YS_MAX;
  }
  male_schlaeger( WHITE );
  /*
  if ( ab_pressed(LEFT_BUTTON) ) ab_invert(true);   // Invertiere Bildschirmfarben (zum Testen)
  if ( ab_pressed(RIGHT_BUTTON) ) ab_invert(false); // Bildschirmfarben wieder normal (zum Testen)
  */
}
...
void bewege_schlaeger_ai1() {
  ...
  if ( xb <= XB_MIN ) {
    ...
    ys1 = ab_random( yb-SCHLAEGER_BREITE+1-AI_Intervallbreite_delta_einseitig_neu, yb+BALL_DURCHMESSER-1+1+AI_Intervallbreite_delta_einseitig_neu); 
    if ( ys1 < YS_MIN ) ys1 = YS_MIN;
    if ( ys1 > YS_MAX ) ys1 = YS_MAX;
    male_schlaeger(WHITE);
  }
...
void bewege_schlaeger_ai2() {
  ...
  if ( xb >= XB_MAX ) {
    male_schlaeger(BLACK);
    ys2 = ab_random( yb-SCHLAEGER_BREITE+1-AI_Intervallbreite_delta_einseitig_neu, yb+BALL_DURCHMESSER-1+1+AI_Intervallbreite_delta_einseitig_neu);
    if ( ys2 < YS_MIN ) ys2 = YS_MIN;
    if ( ys2 > YS_MAX ) ys2 = YS_MAX;
    male_schlaeger(WHITE);
  }
...
void male_schlaeger(bool color) {
  ab_drawFastVLine(XS1, ys1, SCHLAEGER_BREITE, color);
  ab_drawFastVLine(XS2, ys2, SCHLAEGER_BREITE, color);
}
...
void pruefe_treffer() {
  ...
  if ( xb <= XB_MIN ) {
    if ( ( yb + BALL_DURCHMESSER < ys1 ) || ( yb > ys1 + SCHLAEGER_BREITE ) ) {   // Falls linker Schläger nicht den Ball trifft...
      if (sound) ab_tone(TON_PUNKT_FREQ,TON_PUNKT_DAUER);  
      punkte2++;                                                                  // ... erhöhe Punkte für Gegner
      if (punkte2 < punkteMax) ab_delay(PUNKTGEWINN_VERZOEGERUNG);                   // ... warte kurz mit dem Countdown (falls Spiel noch nicht zu Ende)    
      ...
      if (sound) ab_tone(TON_SCHLAEGER_FREQ,TON_SCHLAEGER_DAUER);      
  ...
  if ( xb >= XB_MAX ) {
    if ( ( yb + BALL_DURCHMESSER < ys2 ) || ( yb > ys2 + SCHLAEGER_BREITE ) ) {   // Falls linker Schläger nicht den Ball trifft...
      ...
      if (punkte1 < punkteMax) ab_delay(PUNKTGEWINN_VERZOEGERUNG);                     // ... warte kurz mit dem Countdown (falls Spiel noch nicht zu Ende)    
...
void fuehre_anstoss_aus() {
  ...
  if (sound) ab_tone(TON_KICKOFF_FREQ, TON_KICKOFF_DAUER); // Und los geht das Spiel (mit Kickoffsound bei erster Ballbewegung (falls sound==true))
}
...
void setup() {
//  ab_begin();
  ab_beginNoLogo();                                      // Initialisiere arduboy, aber zeige kein Logo
  ab_setFrameRate(FPS);                                  // Setze die Framerate
  ab_initRandomSeed();                                   // Initialisiere den Pseudozufallszahlengenerator, so dass bei jedem Start andere "Zufallszahlen" für Abschlag, AI, etc. geählt werden.
}
